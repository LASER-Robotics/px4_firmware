...
int nn_example_main(int argc, char *argv[])
{
	...
	orb_set_interval(actuator_controls_sub, 50);

    // subscribe to sensor_combined topic
	int actuator_outputs_sub = orb_subscribe(ORB_ID(actuator_outputs));
	// limit the update rate to 5 Hz
	orb_set_interval(actuator_outputs_sub, 50);
	...
	px4_pollfd_struct_t fds[] = {
        ...
        { .fd = actuator_outputs_sub,   .events = POLLIN },
    ...
    for(int i = 0; i < 100; i++){
        ...
        /* handle the poll result */
            ...
            if (fds[0].revents & POLLIN) {
                ...
                struct actuator_outputs_s raw_out;
                orb_copy(ORB_ID(actuator_outputs), actuator_outputs_sub, &raw_out);
                ...
                (double)values[1] - (double)raw_out.output[1],
                (double)values[2] - (double)raw_out.output[2],
                (double)values[3] - (double)raw_out.output[3]);
